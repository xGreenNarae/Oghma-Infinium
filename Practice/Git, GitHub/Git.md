...
(남겨둘 4번째커밋까지 같이 잡아야 한다)
`git rebase -i 1234`  
또는
`git rebase -i HEAD~4` (이 쪽이 편리하다.)
그러면 위에서부터 1st, 2nd, 3rd .. 로 역순으로 표기될것.(**가장 아래있는것이 최근커밋**) 

순서를 바꿔도 되고.. 커밋 이름도 변경할수있다.  
없앨커밋은 pick -> f 로 바꾸고, 남길것만 pick을 두자.
맨 위의 commit은 squash 할 수 없다. pick으로 남겨둬야 한다.

squash 를 입력했을 경우, 커밋 메시지를 수정하는 창이 한번 더 열리게 된다.
